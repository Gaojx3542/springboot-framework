...
import com.newlandpay.newretail.appstore.utils.tlv.SimpleTLVPackage;
import com.newlandpay.newretail.appstore.utils.tlv.TLVPackage;
...
public class ISOUtils {
    ...
    */
     ...
     * @return
     */
    public static String bcdToDecimalStr(byte[] b, int offset,
            int len, boolean padLeft)
	{
		StringBuilder d = new StringBuilder(len);
		int start = (((len & 1) == 1) && padLeft) ? 1 : 0;
    ...
    public static byte[] bitSet2byte (BitSet b, int bytes)
    {
        ...
        //review why 2nd & 3rd bit map flags are set here???
        if (len > 64) {
     ...
     */
    public static int parseInt (String s, int radix) throws RuntimeException {
        int length = s.length();
        if (length > 9) {

            throw new RuntimeException("Number can have maximum 9 digits");
        }
        int result = 0;
        int index = 0;
        ...
        if (digit == -1) {
            ...
            throw new RuntimeException("String contains non-digit");
        }
        result = digit;
        while (index < length) {
            result *= radix;
     ...
     */
    public static int parseInt (char[] cArray, int radix) throws RuntimeException {
        ...
        if (digit == -1) {
            ...
            throw new RuntimeException("Char array contains non-digit");
        }
        result = digit;
        while (index < length) {
            result *= radix;
     ...
     * @throws RuntimeException
     */
    public static int parseInt (byte[] bArray, int radix) throws RuntimeException {
        ...
        if (digit == -1) {
            ...
            throw new RuntimeException("Byte array contains non-digit");
        }
        result = digit;
        while (index < length) {
            result *= radix;
    ...
    public static double parseAmountConversionRate(String convRate) {
        if (convRate == null || convRate.length() != 8) {
            ...
            throw new RuntimeException("Invalid amount converion rate argument: '" +
                    convRate + "'");
        }
    ...
    public static int unPackIntFromBytes(byte[] lenBytes,int offset,int len,boolean isBigEndian){
    	if(len > 4){

    		throw new RuntimeException("len should be under 4!");}
    	if(len == 4){
    		int last = lenBytes[offset] & 0xff;
    		if(isBigEndian){
    			last = lenBytes[offset + len] & 0xff;
    		}
    		if(last >= 8){
    			throw new IllegalArgumentException("illegal input:"+Dump.getHexDump(lenBytes, offset, len));
    		}
    	}
    ...
    public static byte[] packIntToBytes(int value,int len,boolean isBigEndian){
    	if(len > 4){
    		...
    		throw new RuntimeException("len should be under 4!");}
    	if(value < 0){
    		throw new RuntimeException("illegal input:"+value);
    	}
    ...
    public static int toBERTLVTag(int tag){
        if (tag < 0) {
            ...
            throw new RuntimeException("illegal input:" + tag);
        }
    	...
    	if(rslt > 0xFFFFFF){ //max only to 3 bytes
    		throw new RuntimeException("only support 3 bytes tag" + Dump.getHexDump(ISOUtils.packIntToBytes(tag, 4, true)));
    	}
    	if(rslt > 0xFFFF){
    		if((rslt & 0x1F8000) != 0x1F8000){
    			throw new RuntimeException("illegal tag format:"+Dump.getHexDump(ISOUtils.packIntToBytes(tag, 3, true)));
    		}
    	}
    	if(rslt > 0xFF){
    		if((rslt & 0x1F00) != 0x1F00){
    			throw new RuntimeException("illegal tag format:"+Dump.getHexDump(ISOUtils.packIntToBytes(tag, 2, true)));
    		}
    	}
    	if((rslt & 0x1F) == 0x1F){
    		throw new RuntimeException("illegal tag format:"+Dump.getHexDump(ISOUtils.packIntToBytes(tag, 1, true)));
    	}
    	return rslt;
    }
