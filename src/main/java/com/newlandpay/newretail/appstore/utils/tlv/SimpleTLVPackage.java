...
import com.newlandpay.newretail.appstore.utils.ISOUtils;
...
public class SimpleTLVPackage implements Serializable,TLVPackage {
    ...
    public void append(int tag, String value) {
        append(new SimpleTLVMsg(tag, ISOUtils.hex2byte(value)));
    }
    ...
    public TLVMsg find(int tag) {
        int i = 0;
        tagToFind = ISOUtils.toBERTLVTag(tag);
        while (i < tags.size()) {
    ...
    private TLVMsg getTLVMsg(ByteBuffer buffer) {
        ...
        if (tag !=0) {
            ...
            if (!buffer.hasRemaining()) {
            	throw new RuntimeException("BAD TLV FORMAT - tag ("+Integer.toHexString(tag)+") without length or value");
            }
            else {
                int length = getValueLength(buffer);
                if (length > buffer.remaining()) {

                    throw new RuntimeException("BAD TLV FORMAT - tag (" + Integer.toHexString(tag) + ") length (" + length + ") exceeds available data.");
                }
